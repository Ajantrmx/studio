...
const anomalyAlertPrompt = ai.definePrompt({
  ...
  input: {
    schema: z.object({
      senderCurrentLocationDisplay: z.string(),
      inactivityDurationMinutes: z.number().optional(),
      isOutsideSafeZone: z.boolean(),
      safeZoneDescription: z.string().optional(),
      receiverName: z.string().optional(),
      senderName: z.string().optional(),
      anomalyDetected: z.boolean(),
      anomalyTypesDetected: z.array(z.enum(['inactivity', 'out_of_safe_zone'])).optional(),
    })
  },
...
Yanıtınızda 'anomalyDetected' ve 'anomalyType' alanlarını sağlanan bilgilere göre doğru bir şekilde ayarlayın.`
});
...
const aiAnomalyAlertForReceiverFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (senderLocationHistory.length > 0) {
      ...
      relevantLocations.push(senderCurrentLocation);
    ...
    const safeZoneDescription = "belirlenen güvenli alan";
    ...
    const { output } = await anomalyAlertPrompt({
      ...
      senderName,
      anomalyDetected,
      anomalyTypesDetected: anomalyTypes,
    });
    ...
    return {
      ...
      anomalyType: anomalyTypes.length > 0 ? anomalyTypes : undefined,
    };
  }
);